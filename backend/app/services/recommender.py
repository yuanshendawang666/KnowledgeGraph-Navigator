...
from app.models import KnowledgeStatus, UserKnowledgeProgress, KnowledgePoint
...
class LearningPathRecommender:
    ...
    @staticmethod
    def calculate_module_progress(course_id: int, user_id: int) -> List[Dict]:
        """
        计算模块级别的学习进度。

        算法：
          - 叶子知识点的进度直接从 user_knowledge_progress 表读取
          - 模块的进度 = 所有子孙叶子的 mastery_level 平均值
          - 模块状态: 所有叶子 mastered → mastered,
                      任一叶子 in_progress → in_progress,
                      全 not_started → not_started
        """
        db = SessionLocal()
        try:
            # 获取课程所有知识点（含树结构）
            ...
            kp_map = {kp.id: {
                "id": kp.id, "name": kp.name, "neo4j_node_id": kp.neo4j_node_id,
                "parent_id": kp.parent_id, "level": kp.level, "is_module": kp.is_module,
            } for kp in all_kps}
            ...
            progress_map = {r.knowledge_point_id: r for r in records}

            # 递归计算每个节点的进度
            children_map: Dict[int, List[int]] = {}
            for kp in all_kps:
                if kp.parent_id:
                    children_map.setdefault(kp.parent_id, []).append(kp.id)

            def calc_node(kp_id: int) -> dict:
                kp = kp_map[kp_id]
                children = children_map.get(kp_id, [])

                if not children:
                    # 叶子节点：直接用 user 进度
                    pr = progress_map.get(kp_id)
                    return {
                        **kp,
                        "status": pr.status.value if pr else "not_started",
                        "mastery_level": pr.mastery_level if pr else 0.0,
                        "children": [],
                    }

                # 模块节点：聚合子节点
                child_results = [calc_node(cid) for cid in children]
                levels = [c["mastery_level"] for c in child_results]
                avg_level = sum(levels) / len(levels) if levels else 0.0

                # 模块状态判定
                all_mastered = all(c["status"] == "mastered" for c in child_results)
                any_in_progress = any(c["status"] == "in_progress" for c in child_results)
                any_mastered = any(c["status"] == "mastered" for c in child_results)

                if all_mastered:
                    status = "mastered"
                elif any_in_progress or any_mastered:
                    status = "in_progress"
                else:
                    status = "not_started"
                ...
                return {
                    **kp,
                    "status": status,
                    "mastery_level": round(avg_level, 3),
                    "children": child_results,
                }

            # 从根模块开始计算
            roots = [kp.id for kp in all_kps if kp.parent_id is None]
            return [calc_node(rid) for rid in roots]
