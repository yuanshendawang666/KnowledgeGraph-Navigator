...
class DocumentParser:
    ...
    def _parse_pdf(self, file_path: str) -> str:
        ...
        # 扫描版 PDF（无文字层）→ 尝试 OCR
        from app.services.ocr import get_ocr_service, is_scanned_pdf
        if is_scanned_pdf(extracted, page_count):
            ocr_text = get_ocr_service().extract_pdf(file_path)
            if ocr_text:
                return ocr_text
    ...
    def _parse_image(self, file_path: str) -> str:
        ...
        text = get_ocr_service().extract_image(file_path)
        if not text:
            raise RuntimeError(
                "图片 OCR 识别失败：未安装 PaddleOCR（pip install paddlepaddle paddleocr）"
                "或图片中无文字"
            )
